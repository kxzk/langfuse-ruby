# frozen_string_literal: true

require "spec_helper"
require "opentelemetry/sdk"

RSpec.describe Langfuse::Evaluator do
  let(:tracer_provider) { OpenTelemetry::SDK::Trace::TracerProvider.new }
  let(:otel_tracer) { tracer_provider.tracer("test-tracer") }
  let(:otel_span) { otel_tracer.start_span("test-evaluator") }
  let(:evaluator) { described_class.new(otel_span, otel_tracer) }

  describe "#type" do
    it "returns 'evaluator'" do
      expect(evaluator.type).to eq("evaluator")
    end
  end

  describe "#update" do
    it "updates evaluator attributes" do
      evaluator.update(
        output: { overall_score: 0.87, criteria_scores: { accuracy: 0.92 } },
        level: "DEFAULT",
        metadata: { evaluation_method: "automated" }
      )

      span_data = evaluator.otel_span.to_span_data
      expect(JSON.parse(span_data.attributes["langfuse.observation.output"])).to eq({ "overall_score" => 0.87,
                                                                                      "criteria_scores" => { "accuracy" => 0.92 } })
      expect(span_data.attributes["langfuse.observation.level"]).to eq("DEFAULT")
      expect(span_data.attributes["langfuse.observation.metadata.evaluation_method"]).to eq("automated")
    end

    it "returns self for method chaining" do
      result = evaluator.update(output: { test: true })
      expect(result).to eq(evaluator)
    end
  end

  describe "integration with Langfuse.start_observation" do
    it "creates evaluator via start_observation" do
      evaluator_obj = Langfuse.start_observation("test-evaluator", { input: { response: "test" } }, as_type: :evaluator)

      expect(evaluator_obj).to be_a(described_class)
      expect(evaluator_obj.type).to eq("evaluator")
    end
  end

  describe "integration with Span via start_observation" do
    it "creates evaluator as child of span" do
      parent_span = otel_tracer.start_span("parent-span")
      parent_observation = Langfuse::Span.new(parent_span, otel_tracer)

      evaluator_obj = parent_observation.start_observation("nested-evaluator", { input: { response: "test" } },
                                                           as_type: :evaluator)
      expect(evaluator_obj).to be_a(described_class)
      expect(evaluator_obj.trace_id).to eq(parent_observation.trace_id)
    end
  end

  describe "attribute setters" do
    it "supports input setter" do
      evaluator.input = { response: "test" }
      span_data = evaluator.otel_span.to_span_data
      expect(JSON.parse(span_data.attributes["langfuse.observation.input"])).to eq({ "response" => "test" })
    end

    it "supports output setter" do
      evaluator.output = { overall_score: 0.87 }
      span_data = evaluator.otel_span.to_span_data
      expect(JSON.parse(span_data.attributes["langfuse.observation.output"])).to eq({ "overall_score" => 0.87 })
    end

    it "supports metadata setter" do
      evaluator.metadata = { evaluation_method: "automated", version: "v1" }
      span_data = evaluator.otel_span.to_span_data
      expect(span_data.attributes["langfuse.observation.metadata.evaluation_method"]).to eq("automated")
      expect(span_data.attributes["langfuse.observation.metadata.version"]).to eq("v1")
    end

    it "supports level setter" do
      evaluator.level = "WARNING"
      span_data = evaluator.otel_span.to_span_data
      expect(span_data.attributes["langfuse.observation.level"]).to eq("WARNING")
    end
  end

  describe "#id and #trace_id" do
    it "returns hex-encoded span ID" do
      span_id = evaluator.id
      expect(span_id).to be_a(String)
      expect(span_id.length).to eq(16) # 8 bytes = 16 hex chars
      expect(span_id).to match(/\A[0-9a-f]{16}\z/)
    end

    it "returns hex-encoded trace ID" do
      trace_id = evaluator.trace_id
      expect(trace_id).to be_a(String)
      expect(trace_id.length).to eq(32) # 16 bytes = 32 hex chars
      expect(trace_id).to match(/\A[0-9a-f]{32}\z/)
    end
  end

  describe "initialization with attributes" do
    it "sets initial attributes when provided" do
      attrs = { input: { response: "test" }, output: { overall_score: 0.87 }, level: "DEFAULT" }
      evaluator_obj = described_class.new(otel_span, otel_tracer, attributes: attrs)
      span_data = evaluator_obj.otel_span.to_span_data

      expect(JSON.parse(span_data.attributes["langfuse.observation.input"])).to eq({ "response" => "test" })
      expect(JSON.parse(span_data.attributes["langfuse.observation.output"])).to eq({ "overall_score" => 0.87 })
      expect(span_data.attributes["langfuse.observation.level"]).to eq("DEFAULT")
    end
  end
end
